...
export default async function AtletaPage({
  ...
  const [gols, amarelos, vermelhos, jogosDoTime, eventos] = await Promise.all([
    db.eventoPartida.count({ where: { atletaId: id, tipo: "GOL" } }),
    db.eventoPartida.count({ where: { atletaId: id, tipo: "CARTAO_AMARELO" } }),
    db.eventoPartida.count({ where: { atletaId: id, tipo: "CARTAO_VERMELHO" } }),
    db.partida.count({
      where: {
        status: "ENCERRADA",
